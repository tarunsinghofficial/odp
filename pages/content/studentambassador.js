...
export default function StudentAmbassador() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await databases.listDocuments('6489559f6c9337bce433', '648955aaa894e102ce62');
      ...
      <div className="flex flex-col md:flex-row lg:flex-row p-4 bg-green-900 items-center justify-center gap-1 md:gap-60 lg:gap-72">
        <div className="flex flex-col p-4 gap-4 items-center my-12 max-w-3xl text-left">
          <h1 className="text-5xl md:text-7xl lg:text-7xl text-white font-bold">
            Student Ambassador <span className="text-yellow-400">Programs</span>
          </h1>
          <p className="text-lg text-white">
            A student ambassador program is a program where students are selected to represent their school, college, or university and serve as ambassadors to promote their institution to prospective students, families, and the general public. These students act as a liaison between the school and the community and are responsible for sharing information about the institution, answering questions, and conducting tours. 🚀
          </p>
        </div>
